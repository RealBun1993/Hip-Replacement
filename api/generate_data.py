...
# 初始化Flask应用，指定静态文件夹位置
app = Flask(__name__, static_folder='../static')
CORS(app)

# 读取急性肾功能损伤模型数据
kidney_data = pd.read_csv('./api/kidney_train_selected_features.csv', index_col='ID')
...
# 读取输血预测模型数据
blood_data = pd.read_csv('./api/blood_train_selected_features.csv', index_col='ID')
...
def model_summary_kidney():
    ...
    iterations: {kidney_model.get_param('iterations')}
    learning_rate: {kidney_model.get_param('learning_rate')}
    depth: {kidney_model.get_param('depth')}
    random_state: {kidney_model.get_param('random_state')}
    ...
    summary += f"\nModel training score: {kidney_model.score(X_kidney_train, y_kidney_train):.4f}\n"
...
def model_summary_blood():
    ...
    summary += f"\nModel training score: {blood_model.score(X_blood_train, y_blood_train):.4f}\n"
